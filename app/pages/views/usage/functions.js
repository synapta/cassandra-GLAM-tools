    ...
    $.get("/views/usage/tpl/usage.tpl", function (tpl) {
      ...
      $.getJSON(getUrl(FIRST_CALL_LIMIT, type), function (data) {
        ...
        if (FIRST_CALL_LIMIT < TOTAL_IMAGES) {
          $("#right_sidebar_list")
            .off("scroll")
            .scroll(loadMoreOnScroll.bind($("#right_sidebar_list"), type));
        }
        // Manage click
        highlightOnClick();
      });
...
function renderImageListItems(tpl, data, append) {
  ...
  data.forEach(function (file) {
    ...
    file.pages.forEach(function (page) {
      ...
      if (currentWiki !== page.wiki) {
        ...
        // links
        link_obj.wiki_link =
          page.wiki === "wikidatawiki"
            ? `https://wikidata.org/w/index.php?title=${page.title}`
            : `https://${currentWiki.replace("wiki", "")}.wikipedia.org/w/index.php?title=${
              page.title
            }`;
  ...
  // set status to finished rendering
  RENDERING = false;
  // Prevent default when click on "view details"
  $(".view-details-link")
    .off("click")
    .on("click", function (e) {
      e.stopPropagation();
...
function drawStats(stats_data) {
  $("#usage_stat #distinct-media").append(
    "<p>§[messages.distinct-media]§</p > <div><b>" +
      formatter(stats_data.totalImagesUsed) +
      "</b> / <span id='totalMediaNum'></span></div>"
  );
  $("#usage_stat #total-projects").append(
    "<p>§[messages.total-projects-involved]§</p> <b>" + formatter(stats_data.totalProjects) + "</b>"
  );
  $("#usage_stat #total-pages").append(
    "<p>§[messages.total-pages-enhanced]§</p> <b>" + formatter(stats_data.totalPages) + "</b>"
  );
}
