...
function dataviz() {
  ...
  d3.json(data_source, function (error, data) {
    ...
    if (data.nodes.length === 0) {
      $("#category_network_container").append("<h1>§[messages.no-results-filter]§</h1>");
      $("#right-column").hide();
      return;
    }
    ...
    let svg = d3
      .select("#category_network_container")
      .append("svg")
      .attr("viewBox", "0 0 " + width + " " + height);

    let plot = svg.append("g").attr("id", "d3_plot");
    ...
    legendRow
      ...
      .style("opacity", function (d, i) {
        if (data.nodes.length > 100 && i > MAX_LEVEL - 1) return "0.2";
        else return "1";
      })
...
function highlight() {
  ...
  $(".list_item").on("click", function () {
    ...
    if ($(this).hasClass("list_item_active")) {
      ...
      // turn on circle
      let node_selected = $("#category_network_container")
        .find("." + element)
        .children(".circle");
...
function download(category) {
  ...
  $(
    `<a id="download_dataset_link" title="§[messages.download-dataset]§" href="${getUrlDataset()}" download="${getFileName(
      category
